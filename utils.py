...
class OllamaClient:
    ...
    def generate(self, prompt: str, json_mode: bool = True) -> str:
        ...
        payload = {"model": self.model, "prompt": prompt, "stream": False, "format": "json" if json_mode else None}
        try:
...
def export_to_docx(state: Dict[str, Any], file_path: str):
    ...
    doc.add_paragraph(f"Project Type (from Ollama): {state.get('rfp_summary', {}).get('project_type', 'N/A')}")
